...
interface TerritoryControlChartProps {
    history: any[];
    tribes: Tribe[];
    currentTurn: number;
}

const TerritoryControlChart: React.FC<TerritoryControlChartProps> = ({ history, tribes, currentTurn }) => {
    ...
    const chartData = useMemo(() => {
        console.log('🏛️ TERRITORY CHART: Processing data with history length:', history.length);
        
        if (!history || history.length === 0) {
            console.log('🏛️ No historical data - showing current territory only');
            // Show current data only
            const currentData = new Map<string, Array<{turn: number, garrisons: number}>>();
            tribes.forEach(tribe => {
                const currentGarrisons = Object.keys(tribe.garrisons || {}).length;
                currentData.set(tribe.id, [{
                    turn: currentTurn,
                    garrisons: currentGarrisons
                }]);
            });
            return currentData;
        }
        ...
        const data = new Map<string, Array<{turn: number, garrisons: number}>>();
                ...
                turnRecord.tribeRecords.forEach((tribeRecord: any) => {
                    ...
                    if (tribeData) {
                        tribeData.push({
                            ...
                            garrisons: tribeRecord.garrisons || 0
                        });
                    }
                });
            }
        ...
        console.log('🏛️ Territory chart data processed:', {
    ...
    // Calculate scales
    const { maxGarrisons, turnDomain } = useMemo(() => {
        let maxGarrisons = 0;
        ...
        chartData.forEach(tribeData => {
            tribeData.forEach(point => {
                maxGarrisons = Math.max(maxGarrisons, point.garrisons);
        ...
        return {
            maxGarrisons: Math.max(maxGarrisons, 10), // Minimum scale
    ...
    const yScale = (garrisons: number) => {
        return innerHeight - (garrisons / maxGarrisons) * innerHeight;
    };

    // Generate Y-axis ticks
    const yTicks = useMemo(() => {
        const tickCount = 6;
        const step = Math.ceil(maxGarrisons / tickCount);
        return Array.from({ length: tickCount + 1 }, (_, i) => i * step).filter(v => v <= maxGarrisons);
    }, [maxGarrisons]);
    ...
    return (
        <div className="w-full">
            <div className="mb-4 text-sm text-slate-400">
                Territory expansion and contraction patterns over time
            </div>
                ...
                <svg width={chartWidth} height={chartHeight} className="bg-slate-800 rounded">
                    <g transform={`translate(${margin.left}, ${margin.top})`}>
                        ...
                        <g>
                            ...
                            <text x={-40} y={innerHeight / 2} textAnchor="middle" fill="#94a3b8" fontSize="12" transform={`rotate(-90, -40, ${innerHeight / 2})`}>
                                Garrisons
                            </text>
                        </g>
                        ...
                        <g>
                            {Array.from(chartData.entries()).map(([tribeId, data]) => {
                                ...
                                const pathData = data.map((p, i) => `${i === 0 ? 'M' : 'L'}${xScale(p.turn)},${yScale(p.garrisons)}`).join(' ');
                ...
                <div className="absolute top-4 right-4 bg-slate-700 rounded p-3 max-h-80 overflow-y-auto">
                    ...
                    <div className="space-y-1">
                        {tribes.slice(0, 10).map(tribe => {
                            ...
                            const currentGarrisons = tribeData && tribeData.length > 0 ? tribeData[tribeData.length - 1].garrisons : 0;
...
export default TerritoryControlChart;
